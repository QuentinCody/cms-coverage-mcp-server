...
interface QueryEnv {
    CMS_COVERAGE_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("CMS_COVERAGE_DATA_DO", "cms_coverage");

    server.registerTool(
        "cms_coverage_query_data",
        {
            title: "Query Staged CMS Coverage Data",
            description:
                "Query staged CMS Medicare Coverage data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
