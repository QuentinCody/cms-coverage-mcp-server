...
import { CmsCoverageDataDO } from "./do";

export { CmsCoverageDataDO };

interface CmsCoverageEnv {
...
export class MyMCP extends StatelessMcpWorker {
    ...
    async init() {
        ...
        const env = this.env as unknown as CmsCoverageEnv;
...
export default {
    fetch(request: Request, env: Env, ctx: ExecutionContext) {
        ...
        if (url.pathname === "/health") {
            return buildHealthResponse("cms-coverage");
        }
        ...
        if (url.pathname === "/readyz") {
            ...
            return MyMCP.readiness(env, "cms-coverage");
        }
